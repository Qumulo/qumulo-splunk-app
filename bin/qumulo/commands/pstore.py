...
class PostPstoreFill(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            "--wal_txns",
            help="Optionally, commit additional txns to the WAL.",
            type=int, default=0)
        parser.add_argument(
            "--fast",
        ...
        parser.add_argument(
            "--fragmented",
            help="When filling, do not allocate contiguous runs of blocks",
            action="store_true",
            required=False)
    ...
    def main(conninfo, credentials, args):
        pstore.pstore_fill_post(
            conninfo, credentials, args.pstore, args.unexpired_percentage,
            args.expired_percentage, args.wal_txns, args.fast, args.fragmented)
